...
from .ai_housekeeper import AIHousekeeper, LLMClient
...
class FundManagerShell(cmd.Cmd):
    ...
    def __init__(
        ...
        data_template: str,
        data_hub: LocalDataHub,
        housekeeper: Optional[AIHousekeeper] = None,
    ):
        super().__init__()
        self.db = db
    ...
    def _resolve_csv(self, ticker: str) -> Optional[str]:
        candidate = Path(self.data_template.format(ticker=ticker))
        if candidate.exists():
            return str(candidate)
        hub_path = self.data_hub.market_path(ticker)
        if hub_path.exists():
            return str(hub_path)
        return None

    def do_data(self, arg: str) -> None:
        """data <list|ingest|describe> [...] -- manage the local data hub."""

        tokens = [tok for tok in arg.split() if tok]
        if not tokens:
            print("Usage: data list | data ingest TICKER PATH | data describe TICKER")
            return
        command = tokens[0].lower()
        if command == "list":
            table = self.data_hub.summary_table()
            if table.empty:
                print("Data hub is currently empty.")
            else:
                print(table.to_string(index=False))
            return
        if command == "ingest":
            if len(tokens) < 3:
                print("Usage: data ingest TICKER PATH")
                return
            ticker, path = tokens[1].upper(), tokens[2]
            try:
                dest = self.data_hub.ensure_market_csv(ticker, path)
            except Exception as exc:  # pragma: no cover - interactive feedback
                print(f"Failed to ingest {ticker}: {exc}")
                return
            print(f"Stored market data for {ticker} at {dest}")
            return
        if command == "describe":
            if len(tokens) < 2:
                print("Usage: data describe TICKER")
                return
            ticker = tokens[1].upper()
            records = self.data_hub.describe(ticker)
            if not records:
                print(f"No artefacts recorded for {ticker}.")
            else:
                for rec in records.values():
                    print(
                        f"{rec.kind:<14} -> {rec.path} (updated {rec.updated_at:%Y-%m-%d %H:%M})"
                    )
            return
    ...
    def do_score(self, arg: str) -> None:
        ...
        for ticker in tickers:
            csv_path = self._resolve_csv(ticker)
            if csv_path is None:
                print(f"Data for {ticker} not found. Use 'data ingest' to import it.")
                continue
    ...
    def do_summary(self, arg: str) -> None:  # pylint: disable=unused-argument
        ...
        for holding in holdings:
            csv_path = self._resolve_csv(holding.ticker)
            if csv_path is None:
                print(f"Missing market data for {holding.ticker}; skipping.")
                continue
            try:
    ...
    def do_plan(self, arg: str) -> None:
        """plan [TICKER ...] -- generate a data collection checklist."""

        if self.housekeeper is None:
            print("Housekeeper disabled. Re-run with LLM support to enable this feature.")
            return
        tickers = [tok.upper() for tok in arg.split()] if arg else [h.ticker for h in self.db.list_holdings()]
        response = self.housekeeper.plan_requirements(tickers)
        print(response)

    def do_housekeeper(self, arg: str) -> None:  # pylint: disable=unused-argument
        """Launch an interactive session with the AI housekeeper."""

        if self.housekeeper is None:
            print("Housekeeper disabled.")
            return
        self.housekeeper.interactive_session()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="config/app.yaml", help="Application config file")
    parser.add_argument("--data-root", default="local_data", help="Directory for the local data hub cache")
    parser.add_argument(
        "--disable-housekeeper",
        action="store_true",
        help="Disable the AI housekeeper even if configured",
    )
    parser.add_argument(
        "--housekeeper-provider",
        default=None,
        help="Optional LLM provider (e.g. 'openai'). Defaults to console fallback.",
    )
    parser.add_argument(
        "--housekeeper-model",
        default=None,
...
def build_llm_client(provider: Optional[str], model: Optional[str]) -> Optional[LLMClient]:
    if not provider:
        return None
    ...
    if provider == "openai":
        try:
            ...
            print(f"Failed to initialise OpenAI client ({exc}); using console fallback instead.")
            return None
    ...
    print(f"Unknown LLM provider '{provider}'; defaulting to console mode.")
    return None
...
def main() -> None:
    ...
    macro_cfg = load_json(args.macro_json) or app_config.get("macro", {})
    data_hub = LocalDataHub(args.data_root)
    backend = GNNBackend(
        model_cfg=model_cfg,
        label_cfg=label_cfg,
        window=args.window,
    ...
    coordinator = AnalysisCoordinator(backend=backend, macro_view=macro_cfg, app_config=app_config)
    provider = args.housekeeper_provider
    model = args.housekeeper_model
    if not provider and app_config.get("llm", {}).get("enabled"):
        provider = app_config["llm"].get("provider")
        model = model or None
    llm_client = None if args.disable_housekeeper else build_llm_client(provider, model)
    if not app_config.get("llm", {}).get("enabled"):
        llm_client = None
    housekeeper = None if args.disable_housekeeper else AIHousekeeper(data_hub, llm_client)
    shell = FundManagerShell(
        db=db,
