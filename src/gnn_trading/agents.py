...
class TechnicalAgent(BaseAnalysisAgent):
    ...
    def analyze(self, context: AgentContext) -> AgentReport:
        ...
        chip_conc = float(latest.get("chip_concentration", np.nan))
        chip_support = float(latest.get("chip_support_ratio", np.nan))
        chip_resistance = float(latest.get("chip_resistance_ratio", np.nan))
        chip_main = float(latest.get("chip_main_price", np.nan))

        bullish = int(rsi > 55) + int(macd_hist > 0) + int(slope > 0)
        bearish = int(rsi < 45) + int(macd_hist < 0) + int(slope < 0)
        base_score = (bullish - bearish) / 3.0

        chip_bias = 0.0
        if np.isfinite(chip_support) and np.isfinite(chip_resistance):
            if chip_support - chip_resistance > 0.1:
                chip_bias += 0.4
            elif chip_resistance - chip_support > 0.1:
                chip_bias -= 0.4
        if np.isfinite(chip_conc):
            if chip_conc > 0.3:
                chip_bias += 0.2
            elif chip_conc < 0.15:
                chip_bias -= 0.2

        score = base_score + chip_bias
        confidence = 0.6 + min(abs(compression), 0.4)
        if np.isfinite(chip_conc):
            confidence += min(abs(chip_conc - 0.2), 0.2)
        ...
        if np.isfinite(chip_main):
            insights.append(f"Chip main cost area near {chip_main:.2f}")
        if np.isfinite(chip_conc):
            insights.append(f"Chip concentration {chip_conc:.2%}")
        if np.isfinite(chip_support) and np.isfinite(chip_resistance):
            insights.append(
                f"Support chips {chip_support:.1%} vs resistance chips {chip_resistance:.1%}"
            )
        return AgentReport(
            name=self.name,
            score=score,
            ...
            metrics={
                ...
                "chip_support_ratio": chip_support,
                "chip_resistance_ratio": chip_resistance,
                "chip_main_price": chip_main,
            },
        )
