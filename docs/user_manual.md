...
3. (Optional) Prepare a JSON file mapping `YYYY-MM-DD` to news embedding vectors
   ...
   ```
4. Copy `config_example.yaml` to `config/app.yaml` to centralise runtime flags.
   Leaving the file untouched preserves legacy defaults; enable new features by
   editing the YAML rather than hard-coding CLI arguments.
